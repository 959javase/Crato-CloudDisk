...
async function encryptFile(old_file ,fileKey) {
    ...
    const n = new Promise((resolve, reject) => {
      fr.onload = async () => { 
                console.log(window.atob(DEC.signature).length);
      ...
      fr.onload = async () => { //load        
        ...
        await window.crypto.subtle.decrypt({
          ...
          .then(function (decrypted) {
            resolve(processFinished(file.name.replace('Encrypted-', ''), [new Uint8Array(decrypted)], 2, fileKey)); //create new file from the decrypted content
          })
          .catch(function () {
