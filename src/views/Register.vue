    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          //用户密码加密 MasterKeys
          this.registerForm.master_key_ba = Crypto.encryptAes(this.registerForm.password,master_key)
          console.log(Crypto.decryptAes(this.registerForm.password ,this.registerForm.master_key_ba))
          //console.log( this.registerForm.master_key_ba);
          //生成 RSA 对
          const key = Crypto.getRsaKey()
          //用 MasterKey 加密 RSA 私钥
          this.registerForm.rsa_priv_key_ba = Crypto.encryptAes(master_key,key.privateKey)

          this.registerForm.rsa_pub_key = key.publicKey.replace("-----BEGIN PUBLIC KEY-----","")
