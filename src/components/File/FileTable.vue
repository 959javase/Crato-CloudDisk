<template>
  <div class="file-table-wrapper">
    ...
    <el-table class="file-table"
      ...
      <el-table-column :label="fileType === 6 ? '原路径' : '路径'" prop="filePath" key="filePath"
        show-overflow-tooltip v-if="Number($route.query.fileType)">
        <!-- <template slot-scope="scope">
          <span
          ...
          <span v-show="curRow != null && scope.row == curRow">
            ...
            <el-dropdown @visible-change="changeValue($event)">
              <el-button split-button type="text" style="font-weight:800">
                <i class="el-icon-more el-icon--right" style="font-weight:800"></i>
              </el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item @click.native="handleDeleteFileBtnClick(scope.row)"
                  v-if="deleteBtnShow">删除</el-dropdown-item>
                <el-dropdown-item @click.native="handleRestoreFileBtnClick(scope.row)"
                  v-if="restoreBtnShow">Restore</el-dropdown-item>
                <el-dropdown-item @click.native="handleMoveFileBtnClick(scope.row)"
                  v-if="moveBtnShow">移动</el-dropdown-item>
                <el-dropdown-item @click.native="handleRenameFileBtnClick(scope.row)"
                  v-if="renameBtnShow">重命名</el-dropdown-item>
                <el-dropdown-item v-if="shareBtnShow"
                  @click.native="handleShareFileBtnClick(scope.row)">分享</el-dropdown-item>
                <!-- <el-dropdown-item v-if="downloadBtnShow" @click.native="getDownloadFilePath1(scope.row)">
                  下载
              </el-dropdown-item> -->
                <el-dropdown-item v-if="scope.row.isDir != 1" @click.native="gotoIPFS(scope.row)">
                  在IPFS查看
                </el-dropdown-item>
                <el-dropdown-item v-if="copyLinkBtnShow"
      ...
      <el-table-column label="上传时间" prop="uploadTime" key="uploadTime" width="180"
        :sort-by="['isDir', 'uploadTime']" sortable align="center"
        v-if="selectedColumnList.includes('uploadTime') && ![7, 8].includes(fileType)">
      </el-table-column>
      <el-table-column label="DeleteTime" prop="deleteTime" key="deleteTime" width="180"
        :sort-by="['isDir', 'deleteTime']" sortable align="center"
        v-if="fileType === 6 && selectedColumnList.includes('deleteTime')"></el-table-column>
      ...
      <el-table-column label="分享时间" prop="shareTime" key="shareTime" width="180"
        :sort-by="['isDir', 'shareTime']" show-overflow-tooltip sortable align="center"
        v-if="routeName === 'MyShare'"></el-table-column>
      <el-table-column label="过期时间" prop="endTime" key="endTime" width="190"
        :sort-by="['isDir', 'endTime']" show-overflow-tooltip sortable align="center"
        v-if="routeName === 'MyShare'">
        <template slot-scope="scope">
          <div>
          ...
          <el-dropdown  >
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item
                ...
                @click.native="copyShareLink(scope.row.shareBatchNum, scope.row.extractionCode)">复制链接</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column> -->
    </el-table>
    <el-dialog title="下载" :visible.sync="downloadDialogVisible" width="30%" center>
...
export default {
  ...
  data() {
    return {
      operaColumnExpand: this.getCookies('operaColumnExpand') || false,
  ...
  methods: {
    ...
    gotoIPFS(row) {
      const url = `https://nash-io.cloud/ipfs/${row.cid}?filename=${row.fileName}.${row.extendName}`
    ...
    handleFileNameClick(row, activeIndex) {
      // const mk = this.$store.state.user.mk
      // const fileKey = Crypto.decryptAes(mk , row.fileKey).replace('=','deng')
      // // 若是目录则进入目录
      if (row.isDir) {
        this.$router.push({
          query: {
      ...
      //           return {
      //             fileUrl: this.getViewFilePath(item,fileKey),
      //             downloadLink: this.getDownloadFilePath(item,fileKey),
      ...
      //         imgPreviewList: [
      //           {
      //             fileUrl: this.getViewFilePath(row,fileKey),
      //             downloadLink: this.getDownloadFilePath(row,fileKey),
